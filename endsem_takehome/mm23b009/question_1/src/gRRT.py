...
class RRTAlgorithm:
    ...
    def sampleAPoint(self):
        # Probability of sampling the goal region
        goal_bias = 0.2
        
        if random.random() < goal_bias:
            return np.array([self.goal.locationX, self.goal.locationY])
        else:
            while True:
                ...
                # Calculate the L2 norm distance to the goal
                distance = np.linalg.norm([x - self.goal.locationX, y - self.goal.locationY])
                
                # Probability of sampling based on distance to the goal
...
# Load the grid and create the RRT object
grid = np.load('image.npy')
grid = np.asarray(grid, dtype=int)
start = np.array([530.0, 90.0])
goal = np.array([120.0, 1000.0])
numIterations = 20000
stepSize = 100
goalRegion = plt.Circle((goal[0], goal[1]), 50, color='r', fill=False)
startRegion = plt.Circle((start[0], start[1]), 50, color='r', fill=False)

fig = plt.figure("gRRT path")
